...
#'
#' @description helper function to read in abundance at age reports from EchoPro from different cases
...
Read_Report_Summaries_Abund<-function(SurveyName,DirNameFile,repcases, year, reportpath, hakeflag=2) {
     mlist<-c(1,2) #different methods - 1 is age-1+ (full age) method, and 2 is age-2+ method
     # read in different methods
     #WILL NOW NEED TO LOOP THROUGH HAKEFLAGS
     for (m in mlist){
          method<-m
          # test for which age group wanted - needed for unkriged reports
          if (hakeflag==2){
               Exind<-44  #assume want age-2+ number (is off by 1 number from excel table)
          }else if (hakeflag==4){
               Exind<-43  #assume want age-1+ number(is off by 1 number from excel table)
          }else {
          ...
          if (m==1){ #age-1+ methods
               ...
               print("error in Read_Report_Summaries_Abund: kriged data")
          }
          ...
          if (m==1){ #age-1+ methods
               ...
               print("error in Read_Report_Summaries_Abund: unkriged data")
          }
